...
def _build_sam(
    ...
    if checkpoint is not None:
        ...
        try:
            ...
            #             new_key = new_key[14:]
            #             new_state_dict[new_key] = new_state_dict[k]
            #             _ = new_state_dict.pop(k)
                    
            #         else:
            #             new_state_dict[k[14:]] = new_state_dict.pop(k)


            #     if "prompt_encoder." in k:
            #         new_state_dict[k[15:]] = new_state_dict.pop(k)

            #     if "mask_decoder." in k:
            #         new_state_dict[k[13:]] = new_state_dict.pop(k)


            # sam.image_encoder.load_state_dict(new_state_dict,strict=False)
            # sam.prompt_encoder.load_state_dict(new_state_dict, strict=False)

            # sam.mask_decoder.load_state_dict(new_state_dict,strict=False)
            # sam.mask_decoder2.load_state_dict(new_state_dict,strict=False)
