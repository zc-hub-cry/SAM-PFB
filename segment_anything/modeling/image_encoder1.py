...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.LAFFM_module1 = LAFFM(256)
        self.LAFFM_module2 = LAFFM(256)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # x6 = self.neck(x6.permute(0, 3, 1, 2))
        x6 = self.neck(x6.permute(0, 3, 1, 2))
        x9 = self.neck(x9.permute(0, 3, 1, 2))

        # output =self.LAFFM_module(x,x6)
        output1 =self.LAFFM_module1(x6,x9)
        output =self.LAFFM_module2(x,output1)
        # return output
        return output
...
class LAFFM(nn.Module):
    ...
    def forward(self, x_l , x_r):
        ...
        Q_r_T = self.r_proj1(x_r).permute(0, 2, 1, 3)  # B, H, c, W 
